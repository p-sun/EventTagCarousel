...
public extension UIView {
    ...
    @discardableResult
    public func constrainEdgesHorizontally(to otherview: UIView, offset: CGFloat = 0, usingSafeArea: Bool = false) -> [NSLayoutConstraint] {
        ...
        let constrainable = safeConstrainable(for: otherview, usingSafeArea: usingSafeArea)
        constraints.append(
            constrainLeft(to: constrainable, nil, offset: offset)
        )
        constraints.append(
            constrainRight(to: constrainable, nil, offset: -offset)
        )
        return constraints
    }
